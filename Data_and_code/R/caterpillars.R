...
caterpillars <- function(object, mod = "1",  group, xlab, 
                         ...
                         pred = FALSE, k = TRUE, g = TRUE, at = NULL, by = NULL, weights = "prop") {
  ...
  # Estimate [CI]
  if(mod == "1"){
    x = mean(c(sum_data$x.diamond[1],
               sum_data$x.diamond[3]))
    plot <- plot +
      ggplot2::annotate('text', x = x, y = mod_table$Y[1]-2,
                    label= paste0(
                      round(x,2),
    ...
    x1 = mean(c(sum_data$x.diamond[1],
                sum_data$x.diamond[5]))
    x2 = mean(c(sum_data$x.diamond[2],
                sum_data$x.diamond[6]))
    plot <- plot +
      ggplot2::annotate('text', x = x1, y = mod_table$Y[1]-2,
                        label= paste0(
                          round(x1,2),
                          " [",round(sum_data$x.diamond[1],2),
                        ...
                        size = 3) +
      ggplot2::annotate('text', x = x2, y = mod_table$Y[2]-2,
                        label= paste0(
                          round(x2,2),
